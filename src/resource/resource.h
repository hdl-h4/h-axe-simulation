...
public:
  ...
  void Print() {
    ...
    DLOG(INFO) << " memory = "
               << resource_[static_cast<int>(ResourceType::kMemory)];
    DLOG(INFO) << " disk = "
               << resource_[static_cast<int>(ResourceType::kDisk)];
    DLOG(INFO) << " network = "
               << resource_[static_cast<int>(ResourceType::kNetwork)];
  }

  double GetCPU() const {
    return resource_[static_cast<int>(ResourceType::kCPU)];
  }
  double GetMemory() const {
    return resource_[static_cast<int>(ResourceType::kMemory)];
  }
  double GetDisk() const {
    return resource_[static_cast<int>(ResourceType::kDisk)];
  }
  double GetNetwork() const {
    return resource_[static_cast<int>(ResourceType::kNetwork)];
  }
  ...
  ResourcePack Add(const ResourcePack &rhs) const {
    ResourcePack result;
    result.SetCPU(resource_[static_cast<int>(ResourceType::kCPU)] +
                  rhs.GetCPU());
    result.SetMemory(resource_[static_cast<int>(ResourceType::kMemory)] +
                     rhs.GetMemory());
    result.SetDisk(resource_[static_cast<int>(ResourceType::kDisk)] +
                   rhs.GetDisk());
    result.SetNetwork(resource_[static_cast<int>(ResourceType::kNetwork)] +
                      rhs.GetNetwork());
    return result;
  }
  ...
  friend void from_json(const json &j, ResourcePack &resource_pack) {
    ...
    j.at("cpu").get_to(
        resource_pack.resource_[static_cast<int>(ResourceType::kCPU)]);
    j.at("memory").get_to(
        resource_pack.resource_[static_cast<int>(ResourceType::kMemory)]);
    j.at("disk").get_to(
        resource_pack.resource_[static_cast<int>(ResourceType::kDisk)]);
    j.at("network").get_to(
        resource_pack.resource_[static_cast<int>(ResourceType::kNetwork)]);
  }
